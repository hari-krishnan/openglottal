"""Evaluate BAGLS with GIRAFE z-normalization applied at test time.

Directly apply GIRAFE z-norm statistics to raw BAGLS test pixels.

Operation:
    x_norm = (x/255 - mean_girafe) / std_girafe

This uses the training normalization but on different domain data.

Usage
-----
python scripts/eval_bagls_girafe_znorm.py \\
...
def evaluate(
    ...
    for i, img_path in enumerate(img_files):
        ...
        # Apply GIRAFE z-norm to BAGLS raw pixels
        gray_f = gray_lb.astype(np.float32) / 255.0
        gray_norm = (gray_f - girafe_mean) / girafe_std
        gray_norm = np.clip(gray_norm, 0.0, 1.0)

        # YOLO detector
        if detector is not None:
            detector.reset()
            yolo_img = (gray_norm * 255).astype(np.uint8)
        ...
        mask_u = unet_segment_frame(gray_norm, unet_model, device)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Evaluate BAGLS with GIRAFE z-normalization applied to raw pixels.",
...
def main() -> None:
    ...
    # Load GIRAFE normalization statistics
    ...
    print("Apply GIRAFE z-norm to BAGLS test pixels")
    print(f"GIRAFE z-norm: mean={girafe_mean:.6f}, std={girafe_std:.6f}")
    print("Operation: z = (x/255 - mean) / std")
    print()
