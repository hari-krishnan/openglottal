"""Cross-dataset evaluation on BAGLS with optional z-normalization.

Based on eval_bagls.py but with support for z-score normalization.
Uses normalization parameters estimated from training set.

Usage
-----
With z-normalization (using GIRAFE stats):
  python scripts/eval_bagls_znorm.py \\
    ...
    --znorm-file outputs/normalization_girafe.json \\
    --device mps

Without z-normalization (baseline):
  python scripts/eval_bagls_znorm.py \\
    --bagls-dir /path/to/BAGLS/test \\
    --unet-weights weights/og_girafe_unet_full.pt \\
    --crop-weights weights/og_girafe_unet_crop.pt \\
...
def apply_z_normalization(img_normalized: np.ndarray, mean: float, std: float) -> np.ndarray:
    """
    ...
    img_normalized : ndarray
        Image already normalized to [0, 1] (dtype float32).
    mean, std : float
        Normalization parameters (computed from training set).
    
    Returns
    -------
    Z-normalized image as float32.
    """
...
def unet_on_crop(
    ...
    mask_cs = unet_segment_frame(boxed, model, device, z_norm=z_norm)
...
def evaluate(
    ...
    for i, img_path in enumerate(img_files):
        ...
        # YOLO detection on letterboxed frame.  If z-normalization
        # parameters were provided we must apply the same transform to the
        # image the detector was trained on (the GIRAFE znorm dataset).
        # The normalized training images were grayscale, so we compute a
        # normalized gray frame and then replicate to three channels.
        if detector is not None:
            detector.reset()
            if z_norm is not None:
                # convert to [0,1], apply z-norm, clip and back to uint8
                mean, std = z_norm["mean"], z_norm["std"]
                gray_f = gray_lb.astype(np.float32) / 255.0
                gray_n = (gray_f - mean) / std
                gray_n = np.clip(gray_n, 0.0, 1.0)
                yolo_img = (gray_n * 255).astype(np.uint8)
                # make three channels to satisfy YOLO
        ...
        mask_u = unet_segment_frame(gray_lb, unet_model, device, z_norm=z_norm)
        ...
        if crop_model is not None:
            ...
            if box is not None:
                ...
                mask_c = unet_on_crop(gray_lb, box, crop_model, device, z_norm=z_norm)
            else:
...
def print_table(
    ...
    if z_norm:
        print(f"  Z-norm: mean={z_norm['mean']:.6f}, std={z_norm['std']:.6f}")
    print()
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Cross-dataset evaluation on BAGLS test set with optional z-normalization.",
    ...
    p.add_argument("--znorm-file",    default=None,
                   help="JSON file with z-normalization parameters (mean, std). If provided, applies z-norm.")
...
def main() -> None:
    ...
    z_norm = None
    if args.znorm_file:
        with open(args.znorm_file) as f:
            z_norm = json.load(f)
    ...
    print_table(agg, has_yolo=detector is not None, has_crop=crop_model is not None,
                det_stats=det_stats if detector is not None else None, z_norm=z_norm)
