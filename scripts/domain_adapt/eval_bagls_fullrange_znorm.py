"""Evaluate BAGLS with full-range z-normalized YOLO model.

Apply full-range z-norm (no clipping) to BAGLS test images during inference,
matching the training preprocessing exactly.

Usage
-----
python scripts/eval_bagls_fullrange_znorm.py \\
    --bagls-dir /path/to/BAGLS/test \\
    --unet-weights outputs/openglottal_unet_crop.pt \\
    --yolo-weights outputs/yolo/yolo_znorm_fullrange_girafe/weights/best.pt \\
...
def evaluate(
    ...
    # Full-range z-norm mapping: [-3, +3] → [0, 255]
    z_min, z_max = -3.0, 3.0
    ...
    for i, img_path in enumerate(img_files):
        ...
        # Apply full-range z-norm to BAGLS pixels
        gray_f = gray_lb.astype(np.float32) / 255.0
        gray_znorm = (gray_f - girafe_mean) / girafe_std
        gray_uint8 = ((gray_znorm - z_min) / (z_max - z_min) * 255).astype(np.uint8)

        # YOLO detection
        if detector is not None:
            detector.reset()
            img_for_detector = cv2.cvtColor(gray_uint8, cv2.COLOR_GRAY2BGR)
        ...
        mask_u = unet_segment_frame(gray_uint8, unet_model, device)
        ...
        if crop_model is not None:
            ...
            if box is not None:
                ...
                mask_c = unet_on_crop(gray_uint8, box, crop_model, device)
            else:
                mask_c = np.zeros_like(gray_uint8)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Evaluate BAGLS with full-range z-normalized YOLO.",
    ...
    p.add_argument("--yolo-weights",  required=True,
                   help="YOLO weights (full-range z-norm trained).")
    p.add_argument("--znorm-girafe",  required=True,
                   help="GIRAFE z-norm statistics JSON.")
...
def main() -> None:
    ...
    print(f"Loaded full-range YOLO: {yolo_path}")

    with open(args.znorm_girafe) as f:
        znorm_stats = json.load(f)
    
    girafe_mean = znorm_stats["mean"]
    girafe_std = znorm_stats["std"]
    print(f"Z-norm (GIRAFE)       : mean={girafe_mean:.6f}, std={girafe_std:.6f}")
