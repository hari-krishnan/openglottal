"""Evaluate BAGLS with distribution transfer (denorm BAGLS, renorm GIRAFE).

Domain adaptation via distribution transfer:
    1. Denormalize BAGLS pixels using BAGLS z-norm stats (inverse)
    2. Renormalize using GIRAFE z-norm stats (forward)
    
This maps BAGLS domain into the GIRAFE training distribution space.

Operation:
    x_denorm = (x_norm * std_bagls + mean_bagls)  [undo BAGLS z-norm]
    x_girafe = (x_denorm - mean_girafe) / std_girafe  [apply GIRAFE z-norm]

Usage
-----
python scripts/eval_bagls_dist_transfer.py \\
    --bagls-dir BAGLS/test \\
    --unet-weights outputs/openglottal_unet_crop.pt \\
    --yolo-weights runs/detect/outputs/yolo/yolo_znorm_girafe/weights/best.pt \\
    --znorm-girafe outputs/normalization_girafe.json \\
    --znorm-bagls outputs/normalization_bagls.json \\
    --device mps \\
    --max-images 100
"""
...
def evaluate(
    ...
    for i, img_path in enumerate(img_files):
        ...
        # Distribution transfer: denorm BAGLS -> renorm GIRAFE
        # 1. Convert raw [0,255] to [0,1]
        gray_f = gray_lb.astype(np.float32) / 255.0
        
        # 2. Denormalize using BAGLS stats (inverse z-norm)
        gray_denorm = gray_f * bagls_std + bagls_mean
        
        # 3. Renormalize using GIRAFE stats (forward z-norm)
        gray_transferred = (gray_denorm - girafe_mean) / girafe_std
        gray_transferred = np.clip(gray_transferred, 0.0, 1.0)

        # Apply to YOLO input
        if detector is not None:
            detector.reset()
            yolo_img = (gray_transferred * 255).astype(np.uint8)
            img_for_detector = cv2.cvtColor(yolo_img, cv2.COLOR_GRAY2BGR)
        ...
        # Apply transferred distribution to U-Net
        mask_u = unet_segment_frame(gray_transferred, unet_model, device)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Evaluate BAGLS with distribution transfer (BAGLS→GIRAFE).",
    ...
    p.add_argument("--yolo-weights",   default=None,
                   help="YOLO weights.")
    p.add_argument("--znorm-girafe",   required=True,
                   help="GIRAFE z-norm JSON (mean, std).")
    p.add_argument("--znorm-bagls",    required=True,
                   help="BAGLS z-norm JSON (mean, std).")
...
def main() -> None:
    ...
    girafe_mean = girafe_stats["mean"]
    girafe_std = girafe_stats["std"]
    bagls_mean = bagls_stats["mean"]
    bagls_std = bagls_stats["std"]

    print("Distribution Transfer: BAGLS → GIRAFE")
    print(f"GIRAFE:  mean={girafe_mean:.6f}, std={girafe_std:.6f}")
    print(f"BAGLS:   mean={bagls_mean:.6f}, std={bagls_std:.6f}")
    print("Operation: denorm(x, BAGLS) → renorm(x, GIRAFE)")
    print()
    ...
    agg, det_stats = evaluate(
        ...
        detector      = detector,
        device        = device,
        bagls_mean    = bagls_mean,
        bagls_std     = bagls_std,
        girafe_mean   = girafe_mean,
        girafe_std    = girafe_std,
