"""Cross-dataset evaluation on BAGLS with multiple z-normalization modes.

Allows testing different normalization strategies to understand cross-dataset
generalization challenges. Modes include:
  - none:    Baseline (no z-norm)
  - girafe:  GIRAFE statistics (models trained with this)
  - bagls:   BAGLS statistics (test set distribution)
  - blended: Average of GIRAFE and BAGLS parameters

Usage
-----
python scripts/eval_bagls_multi_znorm.py \\
...
def print_table(
    ...
    if znorm_mode != "none":
        title += f" ({znorm_mode.upper()} z-norm)"
    print(f"\n{sep}")
    print(f"  {title}")
    ...
    if z_norm:
        print(f"  Z-norm [{znorm_mode}]: mean={z_norm['mean']:.6f}, std={z_norm['std']:.6f}")
    print()

    # If YOLO was enabled, print detection precision/recall
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Cross-dataset evaluation on BAGLS test set with multiple z-normalization modes.",
    ...
    p.add_argument("--znorm-mode",    choices=["none", "girafe", "bagls", "blended"], 
                   default="none",
                   help="Z-normalization mode: none (baseline), girafe (GIRAFE params), "
                        "bagls (BAGLS params), blended (average of both).")
    p.add_argument("--znorm-girafe",  default="outputs/normalization_girafe.json",
                   help="JSON file with GIRAFE z-norm parameters.")
    p.add_argument("--znorm-bagls",   default="outputs/normalization_bagls.json",
                   help="JSON file with BAGLS z-norm parameters.")
...
def main() -> None:
    ...
    # Load z-normalization parameters based on mode
    z_norm = None
    if args.znorm_mode != "none":
        if args.znorm_mode == "girafe":
            with open(args.znorm_girafe) as f:
                z_norm = json.load(f)
            print(f"[GIRAFE z-norm] mean={z_norm['mean']:.6f}, std={z_norm['std']:.6f}\n")
        elif args.znorm_mode == "bagls":
            with open(args.znorm_bagls) as f:
                z_norm = json.load(f)
            print(f"[BAGLS z-norm] mean={z_norm['mean']:.6f}, std={z_norm['std']:.6f}\n")
        elif args.znorm_mode == "blended":
            with open(args.znorm_girafe) as f:
                girafe = json.load(f)
            with open(args.znorm_bagls) as f:
                bagls = json.load(f)
            z_norm = {
                "mean": (girafe["mean"] + bagls["mean"]) / 2.0,
                "std": (girafe["std"] + bagls["std"]) / 2.0,
            }
            print(f"[BLENDED z-norm] mean={z_norm['mean']:.6f}, std={z_norm['std']:.6f}\n")
