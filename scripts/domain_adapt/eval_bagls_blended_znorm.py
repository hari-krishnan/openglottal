"""Evaluate BAGLS with blended z-normalization (average of GIRAFE and BAGLS stats).

This script tests domain alignment by using an averaged normalization computed
from both source (GIRAFE) and target (BAGLS) datasets. This reduces the
distribution shift between training and test data.

Usage
-----
python scripts/eval_bagls_blended_znorm.py \\
...
def evaluate(
    ...
    for i, img_path in enumerate(img_files):
        ...
        # Apply z-norm to YOLO input if parameters provided
        if detector is not None:
            detector.reset()
            if z_norm_mean is not None and z_norm_std is not None:
                gray_f = gray_lb.astype(np.float32) / 255.0
                gray_n = (gray_f - z_norm_mean) / z_norm_std
        ...
        # Apply z-norm to U-Net input
        if z_norm_mean is not None and z_norm_std is not None:
            gray_f = gray_lb.astype(np.float32) / 255.0
            gray_n = (gray_f - z_norm_mean) / z_norm_std
            gray_n = np.clip(gray_n, 0.0, 1.0)
            mask_u = unet_segment_frame(gray_n, unet_model, device)
        else:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Evaluate BAGLS with blended (averaged) z-normalization.",
...
def main() -> None:
    ...
    # Blended statistics (average)
    blended_mean = (girafe_mean + bagls_mean) / 2.0
    blended_std = (girafe_std + bagls_std) / 2.0

    print(f"GIRAFE:  mean={girafe_mean:.6f}, std={girafe_std:.6f}")
    print(f"BAGLS:   mean={bagls_mean:.6f}, std={bagls_std:.6f}")
    print(f"Blended: mean={blended_mean:.6f}, std={blended_std:.6f}")
    print()
    ...
    agg, det_stats = evaluate(
        ...
        detector      = detector,
        device        = device,
        z_norm_mean   = blended_mean,
        z_norm_std    = blended_std,
