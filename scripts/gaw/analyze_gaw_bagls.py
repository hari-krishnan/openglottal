"""GAW feature extraction using the best in-distribution (BAGLS) model.

Identical to analyze_gaw.py but defaults to:
  * BAGLS-trained localizer  (og_bagls_yolo.pt)
  * BAGLS full-frame segmenter (og_bagls_unet_full.pt)
  * confidence threshold τ = 0.35

Output is saved to results/gaw/gaw_features_bagls_tau035.json so it
does not overwrite the existing GIRAFE-model run.

Usage
-----
python scripts/analyze_gaw_bagls.py \\
...
DEFAULT_YOLO  = "weights/og_bagls_yolo.pt"
DEFAULT_UNET  = "weights/og_bagls_unet_full.pt"
DEFAULT_CONF  = 0.35
DEFAULT_OUT   = "results/gaw/gaw_features_bagls_tau035.json"
...
def extract_gaw_features(
    ...
    if feats is not None and feats.get("f0") is not None:
        feats["f0"] = feats["f0"] * capture_fps
    return feats


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="GAW feature extraction with BAGLS in-distribution model (τ=0.35).",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--raw-data-dir", required=True,
                   help="Directory containing patient*/patient*.avi + metadata.json")
    p.add_argument("--yolo-weights", default=DEFAULT_YOLO)
    p.add_argument("--unet-weights", default=DEFAULT_UNET)
    p.add_argument("--conf",   type=float, default=DEFAULT_CONF,
                   help="YOLO confidence threshold (τ)")
    p.add_argument("--device", default="cpu")
    p.add_argument("--output-file", default=DEFAULT_OUT)
    p.add_argument("--fps", type=float, default=GIRAFE_CAPTURE_FPS,
                   help="Capture frame rate in Hz (GIRAFE HSV = 4000).")
...
def main() -> None:
    ...
    unet.eval()
    print(f"Loaded localizer  : {yolo_path}  (τ={args.conf})")
    print(f"Loaded segmenter  : {unet_path}\n")
    ...
    for pdir in sorted(raw_dir.iterdir()):
        ...
        if feats is None:
            print("    → silent / no glottis detected, skipping.")
            continue
